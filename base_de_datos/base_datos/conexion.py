...
class Conexion:
    ...
    def crear_tabla_usuario(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS usuarios( dni INTEGER,nombre TEXT, usuario TEXT, rol TEXT)")
    ...
    def editar_usuario(self,dni,nombre,usuario,rol):
        self.cursor.execute("UPDATE usuarios SET nombre=?, usuario=?, rol=? WHERE dni=?",(nombre,usuario,rol,dni))
        self.conexion.commit()
